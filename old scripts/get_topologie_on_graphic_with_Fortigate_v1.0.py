...
def get_device_info(ip):
    ...
    for device_type in device_types:
        try:
            ...
            if device_config['device_type'] == 'fortinet':
                # Pour Fortigate, utiliser la commande get system status
                hostname_output = conn.send_command("get system status", expect_string=r"#", read_timeout=20)
                hostname = "Unknown"
                ...
                # Pour les autres équipements (Cisco)
            ...
            if device_config['device_type'] == 'fortinet':
                # Parsing spécifique pour Fortigate
                ...
                for line in lines:
                    ...
                    # Début d'un nouveau bloc d'hôte
                    if line.startswith('vd root/0') or line.startswith('  vd root/0'):
                        # Si on a déjà collecté des infos pour un appareil précédent et qu'il avait une interface, l'ajouter.
                        # La détection de l'interface déclenchera normalement l'ajout, mais c'est une sécurité.
                        if 'interface' in current_device_info:
                             neighbor_name = current_device_info.get('host') or current_device_info.get('ip') or current_device_info.get('mac')
                             if neighbor_name:
                                 neighbor_name = re.sub(r'[^\w\-\.]', '', neighbor_name) # Nettoyer le nom d'hôte
                                 neighbors.append([neighbor_name, current_device_info['interface'], 'Unknown'])
                        
                        # Réinitialiser pour le nouvel appareil
                        current_device_info = {}
                        
                        # Extraire le MAC de la ligne vd root/0
                        parts = line.split()
                        if len(parts) > 2:
                            current_device_info['mac'] = parts[2]
                            
                    # Ligne contenant l'IP
                    elif 'ip ' in line and 'src' in line:
                         try:
                            ip_match = re.search(r'ip (\S+)', line)
                            if ip_match:
                                current_device_info['ip'] = ip_match.group(1)
                         except:
                             pass # Ignorer les erreurs de regex

                    # Ligne contenant le hostname
                    elif "host '" in line:
                        try:
                            host_match = re.search(r"host '([^']+)'", line)
                            if host_match:
                                current_device_info['host'] = host_match.group(1)
                        except:
                            pass # Ignorer les erreurs de regex

                    # Ligne contenant l'interface (port)
                    elif any(part.startswith('port') for part in line.split()):
                         # Extraire l'interface
                        for part in line.split():
                            if part.startswith('port'):
                                current_device_info['interface'] = part
                                
                                # On a trouvé l'interface, cet appareil est complet pour l'instant, on l'ajoute
                                neighbor_name = current_device_info.get('host') or current_device_info.get('ip') or current_device_info.get('mac')
                                if neighbor_name:
                                    neighbor_name = re.sub(r'[^\w\-\.]', '', neighbor_name) # Nettoyer le nom d'hôte
                                    neighbors.append([neighbor_name, current_device_info['interface'], 'Unknown'])
                                
                                # Réinitialiser les infos de l'appareil courant après l'avoir ajouté
                                current_device_info = {}
                                break # Sortir de la boucle interne une fois l'interface trouvée et le voisin ajouté
                
                # Après avoir parcouru toutes les lignes, le dernier appareil pourrait ne pas avoir eu de ligne 'port' détectée en dernier pour le déclenchement final.
                # Bien que la logique actuelle ajoute au moment de trouver 'port', cette partie est une sécurité si la sortie Fortigate varie.
                # Dans ce cas précis de sortie, la ligne 'port' est toujours la dernière info significative d'un bloc d'appareil.
                # La logique actuelle qui ajoute le voisin dès la détection de 'port' est donc appropriée.
                # On peut laisser cette partie vide ou avec une vérification minimale si nécessaire.

            else:
                # Parsing existant pour les autres équipements (Cisco, etc.)
